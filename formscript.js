...
form.addEventListener("submit", (e) => {
  ...
  if (
    ...
    alert(
      "Organization name, street, city & country, postal address and contact details are required"
    );
    return;
  }
...
const fillInfrastructureDataArray = (tableRowsLength) => {
  ...
  for (let i = 1; i < tableRowsLength; i++) {
    ...
    if (
      !application_name &&
      (operating_system || application_cpu || application_ram || application_storage)
    ) {
      alert(
...
bmsSaveBtn.addEventListener("click", (e) => {
  ...
  if (smallType) {
    bmsArray.push({ bms_id: 1, number_required: smallType });
  }
  if (largeType) {
    bmsArray.push({ bms_id: 2, number_required: largeType });
  }
...
vdiSaveBtn.addEventListener("click", (e) => {
  ...
  if (greaterThanOrEqualTo12Type) {
    vdiArray.push({ vdi_id: 1, number_required: greaterThanOrEqualTo12Type });
  }

  if (sixToLessThan12) {
    vdiArray.push({ vdi_id: 2, number_required: sixToLessThan12 });
  }

  if (lessThan6) {
    vdiArray.push({ vdi_id: 3, number_required: lessThan6 });
  }
...
vmSaveBtn.addEventListener("click", (e) => {
  ...
  for (let i = 0; i < 11; i++) {
    if (no[i] && cs[i].length) {
      vmArray.push({ vm_id: i + 1, number_required: no[i], csbs_required: cs[i] });
    } else {
      continue;
    }
  }
...
submitFormBtn.addEventListener("click", (e) => {
  ...
  // const sim = si.substring(0, 30);
  // const img = signaturePad.fromDataURL(si);
...
const buildObject = (
  ...
  if (infrastructureArray.length > 0) detailsObject = { ...detailsObject, infrastructureArray };
